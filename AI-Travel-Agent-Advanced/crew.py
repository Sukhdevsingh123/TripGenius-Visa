...
#!/usr/bin/env python3
"""
AI Travel Planner - OpenAI Version
A simplified but functional multi-agent travel planning system using GPT-4o-mini
"""
...
class TravelTools:
    ...
    # FIX: Removed the __init__ and the wrapper for SerperDevTool.
    # We will use SerperDevTool directly in the Agents class.
    
    @tool("Calculate travel expenses")
    def calculate_expenses(operation: str) -> str:
        """Calculate travel expenses and budgets. 
        Input should be a mathematical expression string like '200 * 4 + 500'."""
        try:
            # Safety check for mathematical expressions
            allowed_chars = set('0123456789+-*/.() ')
            if not all(c in allowed_chars for c in operation):
...
class TravelPlannerAgents:
    ...
    def __init__(self):
        self.llm = self._initialize_llm()
        # FIX: Instantiate SerperDevTool directly here
    ...
    def _create_agents(self) -> Dict[str, Agent]:
        ...
        # FIX: We pass self.search_tool directly, and TravelTools.calculate_expenses
        ...
        agents['destination_analyst'] = Agent(
            ...
            seasonal events, and matching destinations to traveler preferences. You provide data-driven 
            recommendations with clear reasoning.""",
            tools=[self.search_tool, TravelTools.calculate_expenses],
            llm=self.llm,
            verbose=True,
            allow_delegation=False
        )
...
class TravelPlannerApp:
    ...
    def run_cli(self):
        """Run the command-line interface"""
        print("🌟" + "="*60 + "🌟")
        print("        AI TRAVEL PLANNER (Powered by OpenAI)")
        print("🌟" + "="*60 + "🌟")
